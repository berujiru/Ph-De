...
- **Mood**: A souls-like ruined battlefield — scorched earth, cracked pavement, collapsed buildings and burning wreckage flanking the marching lane, fire glow and drifting smoke. The ruin escalates per act (ashen dawn → smoldering → blood dusk → blackened inferno). The center marching lane always stays readable; ruin and flames live on the shoulders and screen edges. Map backdrops are generated by `scripts/generate-map-skins.mjs` (`npm run gen:maps`) — edit the generator, never the emitted SVGs.
