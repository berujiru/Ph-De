...
This document outlines the required architectural shifts for the gameplay and UI engineers to pivot the game from a static Tower Defense to a "Battle Cats" style Auto-Battler where units march upwards.
...
*   **Summoning Mechanics**: Introduce a `summonHero(heroId)` function in the economy/gameplay core. This deducts cost (Gold/Hope) and spawns the hero at `(randomX_in_lane, screenBottom)`.
*   **Hero State Machine**: Heroes need a simple state machine:
    *   `MARCHING`: Move Y upwards at `speed`.
...
*   The `Targeting.ts` system changes from "find furthest along path" to "find closest enemy in Y-axis distance" or standard auto-battler collision boxes.
*   Ranged heroes look for the first enemy within their Y-range box.

### The Win/Loss Condition
*   **Old**: Enemies leak past the barrier, Lives drop to 0.
*   **New**: If anomalies push the frontline all the way to the bottom edge of the screen, the rally is broken (Game Over).
*   **Win**: Destroy the enemy "System Base" at the top of the screen, or survive all waves (if retaining wave-based survival).

## 2. Entities (`src/game/entities/`)

*   Rename `Tower.ts` to `HeroEntity.ts` (or simply `Hero.ts`).
*   Add `update(dtMs)` logic to `Hero.ts` to handle marching upwards when no enemies are in range.
*   Enemies (`Enemy.ts`) no longer need strict path-following curves. They just march downwards (with slight horizontal wandering/lanes).
