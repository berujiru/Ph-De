...
> rear 3/4)** — top of head/cap and cart lid from above, shoulders, and back. Not
...
> cones). Clean grid, one animation per row, evenly spaced, non-overlapping poses,
...
> 2. **`march`** — EXACTLY 4 frames: a full walk cycle advancing forward, pushing
>    ahead with purpose (rear view).
> 3. **`attack`** — EXACTLY 3 frames: scooping and **flinging a sticky ice-cream
...
>    view).
> 4. **`cast`** — EXACTLY 3 frames: his signature skill **Dirty Ice Cream** —
...
>    scoop, head turned slightly so we catch his cheerful grin (rear-3/4 view).
> 6. **`defeat`** — EXACTLY 2 frames: morale broken — lowering the scoop and
