...
> enemy. **ALL frames must be drawn from a HIGH-ANGLE REAR VIEW (top-behind, back view)** — top of head, the shoulder yoke and canister lids from above,
...
> 2. **`attack`** — EXACTLY 3 frames: scooping from a canister and **lobbing a
...
> 3. **`cast`** — EXACTLY 3 frames: his signature skill **Hot Syrup** — planting
